...
utils::sfc_header::SFCFixedLengthHeader::SFCFixedLengthHeader(const unsigned char* byte_array) noexcept {
    memcpy(&header, byte_array, sizeof(header));
}

utils::sfc_header::SFCFixedLengthHeader::SFCFixedLengthHeader(std::shared_ptr<unsigned char> byte_array) noexcept {
...
std::string utils::sfc_header::SFCFixedLengthHeader::get_source_ip_address() const {
    return SFCUtilities::int_to_ip(header.source_address);
}

std::string utils::sfc_header::SFCFixedLengthHeader::get_destination_ip_address() const {
...
void utils::sfc_header::SFCFixedLengthHeader::set_service_index(uint32_t service_index) {
