...
JavaHandler::JavaHandler(const std::string &config_file)
    ...
    std::string class_file_path = config->getField(utils::externHookFields::FILE_PATH);
    std::string class_name = config->getField(utils::externHookFields::CLASS_NAME);
    std::string method_name = config->getField(utils::externHookFields::METHOD);
...
std::shared_ptr<uint8_t> JavaHandler::handler_request(std::shared_ptr<uint8_t> message, std::size_t size) {
    return execute_java(config->getField(utils::externHookFields::FILE_PATH),
                       config->getField(utils::externHookFields::CLASS_NAME),
                       config->getField(utils::externHookFields::METHOD),
                       message, size);
}
